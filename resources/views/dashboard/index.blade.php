            ...
            <div class="comment d-flex align-items-start justify-content-between">
                ...
                @else 
                    <form action="/todo/completed/{{$item['id']}}" method="POST">
                            @csrf
                            @method('PATCH')
                        <button type="submit" class="fas fa-circle-check task-primary btn"></button>
                    </form>
                @endif
                <div class="d-flex flex-column">
                    ...
                    <span class="date">
                        @if ($item['status'] == 1)
                        diselesaikan saat {{ \Carbon\Carbon::parse($item['done_time'])->format('j F, Y')}}
                        @else
                        target harus selesai {{ \Carbon\Carbon::parse($item['date'])->format('j F, Y')}}
                        @endif
                    </span>

                    <div class="d-grid gap-2 d-md-block">
                        <a href="{{route ('todo.delete', $item->id)}}" button class="btn btn-outline-danger btn-sm mt-2" type="button">Hapus</button></a>
                        <a href="{{route ('todo.edit', $item->id)}}" button class="btn btn-outline-dark btn-sm mt-2" type="button">Edit</button></a>
                      </div>
                </div>
